...
window.onmousedown = function (event) {
  ...
  */
    ...
    if (
      ...
      const r_x = obj.x - radar.x;
      const r_y = obj.y - radar.y;

      const gab = gameFielg[0][0].drawGabaritRadar;
      // drawGabaritRadar

      const reX = r_x / gab;
      const reY = r_y / gab;

      removeX = -(Math.floor(reX) * 50 - 350) + ((Math.floor(reX) * 50 - 350) % 25);
      removeY = -(Math.floor(reY) * 50 - 350) + ((Math.floor(reY) * 50 - 350) % 25);
