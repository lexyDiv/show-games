...
const get_distanse_on_lineyka = function (
  ...
  if ((ot_gabarit === 50 || ot_gabarit === 150) && do_gabarit === 100) {
    const a = do_cell.x + 50 - (ot_cell.x + 25);
    const b = do_cell.y - (ot_cell.y + 25);
    ...
    const a = do_cell.x - ot_cell.x;
    const b = do_cell.y - ot_cell.y;

    distanse = Math.sqrt(a * a + b * b);
  } else if (ot_gabarit === 100 && (do_gabarit === 50 || do_gabarit === 150)) {
    const a = do_cell.x + 25 - (ot_cell.x + 50);
    const b = do_cell.y + 25 - ot_cell.y;

    distanse = Math.sqrt(a * a + b * b);
  } else if (ot_gabarit === 100 && do_gabarit === 100) {
    const a = do_cell.x + 50 - (ot_cell.x + 50);
    const b = do_cell.y - ot_cell.y;
